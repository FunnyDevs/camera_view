...
class Camera1 extends CameraViewImpl implements MediaRecorder.OnInfoListener,
    ...
    private int calcVideoDisplayOrientation(int screenOrientationDegrees)
    {
        if (mCameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            return (mCameraInfo.orientation - screenOrientationDegrees + 360) % 360;
        } else {  // back-facing
    ...
    private void setUpMediaRecorder(String path, int maxDuration, int maxFileSize, boolean recordAudio, CamcorderProfile profile) {
        ...
        mMediaRecorder.setOrientationHint(calcVideoDisplayOrientation(mDisplayOrientation));
    ...
    private void setCamcorderProfile(CamcorderProfile profile, boolean recordAudio) {
        ...
        float widthSize = mPreview.getView().getMeasuredWidth();
        float heightSize = mPreview.getView().getMeasuredHeight();

        Camera.Size videoSize = getPreviewSize(mCameraParameters.getSupportedVideoSizes(), 600,
